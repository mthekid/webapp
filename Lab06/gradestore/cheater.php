...
<html lang="en">
	...
	<p> <?php 
		// if( preg_match("/^4\d{15}$/" , $_POST["creditCard"]) ) s
		// if( preg_match("/^\d{16}$/" , $_POST["creditCard"]) ) {
		// 	print "match credit card";
		// } else {
		// 	print "didn't match creditcard number";
		// } ?>
<br>
	<?php
	...
	?> </p>
		...
		# 통과X : a  martin, -  martin, 231martin, 231-martin
 		#
			 ...
			 <?php } elseif( !preg_match("/^\d{16}$/", $_POST["creditCard"] ) ) {
				 $flag = false; ?> 
			 <h1>Sorry</h1>
			 <p>You didn't provide a valid credit card number. <a href="./gradestore.html">Try again?</a></p>
			<?php	} elseif( $_POST["cc"] == "visa") { 
				 	if ( !(preg_match("/^4\d{15}$/", $_POST["creditCard"]) ) ) {
						$flag = false; 
		 ?>
		 	 <h1>Sorry</h1>
		...
		# if all the validation and check are passed 
		 } } elseif ( $_POST["cc"] == "MasterCard") { 
			 if ( !(preg_match("/^5{1}\d{15}$/", $_POST["creditCard"])) ) {
				$flag = false; 
		?>
 			<h1>Sorry</h1>
